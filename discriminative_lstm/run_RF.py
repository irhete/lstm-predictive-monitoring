...
for dataset_name in datasets:
    ...
    with open(outfile, 'w') as fout:
        ...
        start = time.time()
        X = np.empty((100, max_len, data_dim), dtype=np.float32)
        y = np.zeros(100)
        idx = 0
        ...
        X_reshaped = X.reshape((100, max_len*data_dim))
        print(X[1,:,:].sum(), X_reshaped[1,:].sum())
        ...
        model.compile(loss=loss, optimizer=SGD(lr=learning_rate, momentum=0.9))
